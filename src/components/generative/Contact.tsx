...
export const Contact = ({ type, info, email, social, availability }: ContactProps) => {
  ...
  if (!type && (email || social || availability)) {
    return (
      ...
      >
        <h3 className="text-xl font-bold text-indigo-900 dark:text-indigo-100 mb-6">
          Contact Information
        </h3>
    ...
    >
      ...
      {type === 'social' && info && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-indigo-900 dark:text-indigo-100">{info.github}</span>
          </div>
          <div className="flex items-center gap-3">
            <Linkedin className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            <span className="text-sm text-indigo-900 dark:text-indigo-100">{info.linkedin}</span>
          </div>
          <div className="flex items-center gap-3">
            <Twitter className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            <span className="text-sm text-indigo-900 dark:text-indigo-100">{info.twitter}</span>
          </div>
        </div>
      )}
      ...
      {type === 'availability' && info && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-green-500" />
            <span className="text-sm text-indigo-900 dark:text-indigo-100">{info.status}</span>
          </div>
          <div className="text-xs text-indigo-600 dark:text-indigo-300">
            ...
            <p>Preferred: {info.preferredContact}</p>
          </div>
        </div>
      )}
    </motion.div>
